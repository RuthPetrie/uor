...
IF (STORE EQ 1) THEN BEGIN
 ...
 IF KEYWORD_SET(lakes) THEN !guide.lakes=1
ENDIF 
...
IF (NOT KEYWORD_SET(STORE)) THEN BEGIN
 ...
 IF (NH+SH NE 0) THEN BEGIN
  ...
  ; include /GRID as keyword to show grid lines
;  com='MAP_SET, hem, map_centre, LIMIT=[latmin, lonmin, latmax, lonmax], /NOERASE, /NOBORDER, /GRID'
 ...
 IF (KEYWORD_SET(LAKES) AND (!guide.cthick GT 0)) THEN BEGIN  
