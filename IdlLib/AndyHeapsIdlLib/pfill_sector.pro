PRO PFILL_SECTOR, FIELD=field, X=xpts, Y=ypts, MIN=min, MAX=max, SPACE=space, $
...
IF ((N_ELEMENTS(latmin) EQ 1) OR (N_ELEMENTS(latmax) EQ 1)) $
 ...
 valid_pts=WHERE(data_pts(1,*) GT latmin AND (data_pts(1,*) LT latmax) $
                AND data_pts(0,*) GT  lonmin AND data_pts(0,*) LT lonmax  )
 pts=pts(valid_pts)
