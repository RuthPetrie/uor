...
foreach d {x y} {
  if {[info exists ${d}inc]} {
    ...
    if {$d == "x"} {
      if {$xinc < 0.5} {
        exitandreport "xi argument (which must be greater than 0.5)"
      }
    } elseif {$d == "y"} {
      if {$yinc < 0.5} {
        exitandreport "yi argument (which must be greater than 0.5)"
      }
    }
  }
}
