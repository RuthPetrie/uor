...
/* open files for read and check headers are consistent */
   ...
   fgets(line, 100, fin2);
   sscanf(line, "%d %ld %d %d %d %d %d %d %d %d", &ntrnum, &nptnum, &nntheta, &nnr, &nnwfld, &nidir, &nndsmth, &nifcnt, &nitpadd, &niprojd);

/* check headers */

   if((trnum != ntrnum) || (ptnum != nptnum) || (ntheta != nntheta) || (nr != nnr) || (nwfld != nnwfld) || 
      (idir != nidir) || (ndsmth != nndsmth) || (itpadd != nitpadd) || (iprojd != niprojd)) {
