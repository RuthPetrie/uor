...
void main(void )
{
    ...
    int istc=0;
    int nth=1;
    int ift=0, if1=0, if2=0, ifs=0, ifs2=0;
    int iff1, iff2, ifft;
    ...
    float str=0.0, stl1=0.0, stl2=0.0;
...
/* scaling of advection term assumes we are working with gradients of geopotential in units of m^2 s^(-2) */
             ...
             scanf("\n");
             if(getchar() == 'y') levdat = leveldata(irnf);
...
/* calculate data block size */
       ...
       printf("Do you want field extraction and averaging at:-        \r\n"
              ...
              "a particular time step,           '3'                  \r\n"
              " or at the time of genesis        '4'                  \n\n");
    ...
    if(nff){
       ...
       if(itv == 'y'){
          ...
          scanf("%f", &fth);
          printf("What value do you want for the number of points that satisfy the intensity threshold, \r\n"
                 "fth (warm core) and vertical structure.\n\n");
          scanf("%d", &nth);

          printf("Specify which pair of additonal fields to use for warm core test, \r\n"
                 "'0' implies use of original vortcity center.                      \n\n");
          scanf("%d %d", &if1, &if2);
          ...
          if(getchar() == 'y') {
            ifs = 1;
            if(nvert) exit(1);
            printf("Do you want to check for decreasing vorticity with hight between specified pairs of additional fields\r\n"
                   "between those chosen for warm core, 'y' or 'n'                                                       \n\n");
            scanf("\n");
            if(getchar() == 'y'){
               ifs2 = 1;
               printf("Use all additional fields between levels chosen for warm core check or select levels, '0' for select and '1' for all.\n\n");
               scanf("%d", &ifsel);
               if(!ifsel){
                  printf("How many additional fields/levels, do you want to use?\n\n");
                  scanf("%d", &nlv);
                  if(nlv > nlp){
                     printf("****ERROR****, too many levels specfied.\n\n");
                     exit(1);
                  }
                  lvt = (int *)calloc(nlv, sizeof(int));
                  mem_er((lvt == NULL) ? 0 : 1, nlv * sizeof(int));
                  for(i=0; i < nlv; i++){
                      printf("What is additional field for level %d? ", i+1);
                      scanf("%d", lvt + i);
                      printf("\n");
                      if(*(lvt + i) < if1 || *(lvt + i) > if2){
                         printf("****ERROR****, additional field out of allowed range.\n\n");
                         exit(1);
                      }
                  } 
               }
           ...
           if(ifs){
              if(!if1){
                 ...
                 for(k=0; k < if2; k++){
                     ...
                     stl2 = fp->add_fld[nn+2];
                     if(ifsel && stl1 - stl2 < 0.0){iv = 0; break;}
                     stl1 = stl2;
                     nn += 3;
                 }
              }
...
/* vertical integration */
          ...
          if(levdat) {
             ...
             vlintg1 = vert_integ(lvtmp, vint, nll1, nll2);
             vaintg1 = vert_integ(rintg1, vint, nll1, nll2);
...
/*                for(j=0; j < irdim; j++) *(sradf1 + j) = (float)(*(savg3[i] + j));
             ...
             if(levdat) {
                ...
                vlintg2 = vert_integ(lvtmp, vint, nll1, nll2);
                vaintg2 = vert_integ(rintg2, vint, nll1, nll2);

/*                vkeintg = vert_integ(vke, vint, nll); */

                vkeintg = vert_integ(rintg3, vint, nll1, nll2);
                ...
                if(idevazm){
                   ...
                   vphi = vert_integ(alv, vint, nll1, nll2);
...
double vert_integ(double *dat, double *vlev, int nl1, int nl2)
{
    int i=0;
