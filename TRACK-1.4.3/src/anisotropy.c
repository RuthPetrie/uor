...
float sys_area(struct boundary_pt * , int , int , int , float );
int inside(int , struct boundary_pt * , float , float );
float area_int(struct boundary_pt * , int , float * , float * , int , int , float , float );
...
/*      if(aar == 'y'){
         ...
         printf("****WARNING****, this option is not yet available\n\n");
         aar = 'n';
      }
...
/* compute area of a sub-region */
          ...
          if(aar == 'y' && npt > 1 ){
             ...
             fpts->area = sys_area(ob_area.bound, ob_area.bound_num, iff, atype, fpts->str);
...
/* eight connected search */
       ...
       pta = search_aniso(pta, npt, aa, fpth, nerx+1, nery, xdim, pxmn, pymn, ap, msk);
       pta = search_aniso(pta, npt, aa, fpth, nerx, nery+1, xdim, pxmn, pymn, ap, msk);
       pta = search_aniso(pta, npt, aa, fpth, nerx-1, nery, xdim, pxmn, pymn, ap, msk);
       pta = search_aniso(pta, npt, aa, fpth, nerx, nery-1, xdim, pxmn, pymn, ap, msk);

       pta = search_aniso(pta, npt, aa, fpth, nerx+1, nery+1, xdim, pxmn, pymn, ap, msk);
       pta = search_aniso(pta, npt, aa, fpth, nerx-1, nery+1, xdim, pxmn, pymn, ap, msk);
       pta = search_aniso(pta, npt, aa, fpth, nerx+1, nery-1, xdim, pxmn, pymn, ap, msk);
       pta = search_aniso(pta, npt, aa, fpth, nerx-1, nery-1, xdim, pxmn, pymn, ap, msk);
...
float sys_area(struct boundary_pt *bpt, int nbp, int iff, int atype, float fptstr)
{
   ...
   else if (iff < 0) {     /* free memory for refined grid */
      free(xg);
      free(yg);
      ifirst = 0;
      return 0.0;
   ...
   if(!bpt){
      printf("****ERROR****, no boundary information for determining area\n\n");
      return 0.0;
...
/* Does object straddel a periodic boundary */
   ...
   if(!iper) area = area_int(bpt, nbp, xg, yg, nx, ny, dxrad, sindy);
...
/* find first boundary cross-over point */
       ...
       while(ibf >= 0){
          ...
          area += area_int(btmp, nt, xg, yg, nx, ny, dxrad, sindy);
...
float area_int(struct boundary_pt *bpt, int nbp, float *xg, float *yg, int nx, int ny, float dxrad, float sindy)
{
...
/* nearest grid points */
    ...
    for(i=i1; i <= i2; i++){
        coslat = cos(FP_PI * *(yg + i));
        for(j=j1; j <= j2; j++){
            if(inside(nbp, bpt, *(xg + j), *(yg + i))){
               area += dxrad * sindy * coslat;
            }
