...
void stat_type();
void extract_sdt(struct tot_stat * , float * , float * , int * ,int , int , int , int , PROJ );
...
/* ++++++++++++++++++++++++++++ */
   ...
   if(pgr) { alat = gr1->alat; alng = gr1->alng;}
   ...
   for(i=0; i < trsav->ptnum; i++){
       ...
       *yd = pst->ys;
       ...
       if(ipj == 'y'){
          ...
          switch(gr->prgr){
               case 0:
                  proj_point(proj, xd, yd,  pgr, tpr, gr->prgr, gr->prty);                            
                  pst->ptyp = 0;
                  ++c1;
                  break;
               case 1:
                  if(proj_point(proj, xd, yd, pgr, tpr, gr->prgr, gr->prty, &gr1->alat, &gr1->alng)) 
                     {++c1; pst->ptyp = 0;}
                  else if(gr2){
                     proj_point(proj, xd, yd, pgr, tpr, gr->prgr, gr->prty, &gr2->alat, &gr2->alng);
                     ++c2;
           ...
           if(sty > STNM)
              ...
              if(!nmp) {
                 ...
                 *ity = sty;
                 ...
                 if(sty == 9 || sty == 13) ++stt;
                 extract_sdt(trsav, sp, smth, kern, stt, sty, tpr, pgr, proj);
                ...
                *(ity + st - 1) = sty;
                 ...
                 if(sty == 9 || sty == 13) ++stt;
                 extract_sdt(trsav, sp, smth+st-1, kern+st-1, stt, sty, tpr, pgr, proj);
...
retry:
   ...
   if(tpr != gr->prty || pgr != gr->prgr) proj_group(pgr, tpr);
...
void extract_sdt(struct tot_stat *trsav, float *sp, float *sm, int *kty, int asp, int st, int tpr, int pgr, PROJ proj)
   ...
   switch(st){
      ...
      case 9:
         ...
         for(i=0; i < trsav->ptnum; i++) {
             ...
             if(gr->prgr) vect_proj(pst->xs, pst->ys, &xcmp, &ycmp, proj, pgr, tpr, pst->ptyp);
