...
void additional_fields(struct tot_tr * , int , off_t , FILE * , int );
...
int main(int argc, char **argv)
{
   ...
   int sn1, sn2;
   int reo='n', scmp='n';
   int anty, ty;                   /* switchs for track analysis routines */
...
analy:
       ...
       if(anty == 0) goto tidy;
          ...
          if(ty == 0) goto tidy;
          ...
          else if(ty == 1) additional_fields(all_tr, trackn, pl2, fdatin, iper);
           ...
           printf("Use least squares, '0' or fast spectral transform, '1'\n\n");
           scanf("%d", &sp_typ);
       ...
       else if(anty == 12) compute_vorticity(fdatin, pl2, iper);
