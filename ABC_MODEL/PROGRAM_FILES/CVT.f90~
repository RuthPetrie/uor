...
!*************************************************************************************************
 ...
 SUBROUTINE CVT(InitState, Dims, gen_eigvecs, truncate, acoustic, gravity, balanced, inv_cvt_test, &
                ...
                eigenvects_file, eigenvals_file, st_dev_file,  imp_covs_file, ensnum )
 !******************************************************************************************** 
 ...
 CHARACTER(LEN=*),          INTENT(INOUT)     :: eigenvects_file
 CHARACTER(LEN=*),          INTENT(INOUT)     :: eigenvals_file
 CHARACTER(LEN=*),          INTENT(INOUT)     :: imp_covs_file
 CHARACTER(LEN=*),          INTENT(IN)        :: st_dev_file
 ...
 ! Filenames
 CHARACTER                                    :: eigenvects_filename*56,eigenvals_filename*56
 ...
 PRINT*, 'in cvt'
  ...
  PRINT*, 'eigenvects_file:',eigenvects_file
...
!************************************************************************************************** 
  ...
  ELSE
    ...
    PRINT*, 'Reading Eigenvectors', eigenvects_file
   ! eigenvects_filename = eigenvects_file // '.bin'
   ! eigenvects_filename = '/export/carrot/raid2/wx019276/DATA/CVT/EigVecs_ref.bin'
...
!   PRINT*, eigenvects_filename
    ...
    PRINT*, 'Reading Eigenvalues',eigenvals_file
 !   eigenvals_filename = eigenvals_file//'.bin'
 !  eigenvals_filename = '/export/carrot/raid2/wx019276/DATA/CVT/EigVals_ref.bin'
    PRINT*, eigenvals_filename
    OPEN(56, file = eigenvals_file, form = 'unformatted')
    DO i = 1,nlongs
...
!************************************************************************************************** 
 IF  (gen_imp_cov .EQ. 1) THEN
   ...
   CALL Calc_implied_covs(InitState, Dims, Eigvecs, std_eigvecs, &
                          truncate, acoustic, gravity, balanced, imp_covs_file, ensnum)
 END IF
...
! OutputState % v(360,1:nlevs) = 1500*(OutputState % v(360,1:nlevs))
 ...
 CALL Write_state_2d ('/export/carrot/raid2/wx019276/DATA/CVT/eigenfuncts_ref_'//filenumber//'.nc',&
...
!=================================================================================================== 
 ...
 REAL(wp),                  INTENT(IN)        :: std_eigvecs(1:nvars*nlevs, 1:nlongs )
 CHARACTER(LEN=*),          INTENT(IN)        :: imp_covs_file
 CHARACTER(LEN=*),          INTENT(IN)        :: ensnum
 ...
 ! Declare Local Variables
 !---------------------------
 TYPE(model_vars_type)                        :: OutputState, TempState, en_std
 TYPE(Transform_type)                         :: HozCtrl, VertCtrl
 COMPLEX(wp)                                  :: Ctrl_vect(1:nvars*nlevs, 1:nlongs )
...
! Counters
 INTEGER                                      :: i,j,k, x,z, dump_ctrl_vec, correlations
...
!=================================================================================================== 
 ...
 !* i.e the physical hoz wave no.   *
 !* is zero, the infinte wave; this *
 !* corresponds to k_{index} = 181  *
 !***********************************
 
 ! Initialise L matrix to zero
 !-----------------------------
 k = 181
 Lmat(k,1:nvars*nlevs,1:nvars*nlevs) = 0.0 
 ...
 ! Dump L matrix, for testing purposes only
 !-------------------------------------------
...
!--------------
 ...
 eigenvals_filename = eigenvals_file//'.bin'
   PRINT*, 'eigenvals_filename :',eigenvals_filename
