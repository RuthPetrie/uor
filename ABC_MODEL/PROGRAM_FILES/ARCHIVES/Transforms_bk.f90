...
!===================================================================================================
 ...
 DO k = 1, nlongs
    ktemp = k - 1 - (nlongs/2)
    IF (ktemp .EQ. 0) THEN
       ctrl_state_out % u(1:nlevs,k) =  ctrl_state_out % u(1:nlevs,k)
     ELSE
       ctrl_state_out % u(1:nlevs,k) = (ctrl_state_out % u(1:nlevs,k) * domain ) / (2.0*REAL(ktemp)*pi)
    ENDIF
 ENDDO
 ...
 !      ctrl_state_out % v(:,k) = (negimag * ctrl_state_out % v (:,k)* domain ) / (2.0*(REAL(ktemp))*pi) 
       ctrl_state_out % v(:,k) = exp(phase) * (negimag * ctrl_state_out % v (:,k)* domain ) / (2.0*(REAL(ktemp))*pi) 
    ENDIF
 ENDDO
...
!===================================================================================================
 ...
 DO k = 1, nlongs
     ktemp = k - 1 - (nlongs/2)
   IF (ktemp .EQ. 0) THEN
     ctrl_state_in % u(1:nlevs,k) =  ctrl_state_in % u(1:nlevs,k) 
  ELSE
     ctrl_state_in % u(1:nlevs,k) = 2.0 * (REAL(ktemp)) * pi * ctrl_state_in % u(1:nlevs,k) / domain  
   ENDIF
 ENDDO
...
IMPLICIT NONE
 ...
 DO k = 1, nlongs
     ktemp = k - 1 - (nlongs/2)
   IF (ktemp .EQ. 0) THEN
     ctrl_state_in_hat % u(1:nlevs,k) =  ctrl_state_in_hat % u(1:nlevs,k) 
  ELSE
     ctrl_state_in_hat % u(1:nlevs,k) = 2.0 * (REAL(ktemp)) * pi * ctrl_state_in_hat % u(1:nlevs,k) / domain  
   END IF
 END DO
...
SUBROUTINE Eig_inv(ctrl_state_in, ctrl_vect_out, EigVecs, gen_scale, std_eigvecs)
 ...
 COMPLEX(wp),           INTENT(IN)        :: std_eigvecs(1:nvars*nlevs, 1:nlongs) 
 ...
 DO k = 1,360  ! For each horizontal wavenumber
   ...
   ELSE
     ctrl_state_in % w(1:nlevs,k) = ctrl_state_in % w(1:nlevs,k) * negimag * domain /( 2 * REAL(ktemp) * pi)
     ctrl_state_in % r(1:nlevs,k) = ctrl_state_in % r(1:nlevs,k) * domain / (2 *REAL(ktemp) * pi * sqrtb )
     ctrl_state_in % b(1:nlevs,k) = ctrl_state_in % b(1:nlevs,k) * domain / (2 * REAL(ktemp) * pi * sqrt(Nsq))
   ENDIF
   ...
   ctrl_vect_tmp = MATMUL(ev_tmp_arr,x_tmp_arr )
...
!     ctrl_vect_tmp(z) = ctrl_vect_in(z,k)!*std_eigvecs(z,k)
    ...
    ! Truncate physical modes
    ! ctrl_vect has 5 modes, 
    !--------------------------
!    ctrl_vect_tmp(  1:60)  = (0.0,0.0)
!    ctrl_vect_tmp( 61:120) = (0.0,0.0)
!    ctrl_vect_tmp(121:180) = (0.0,0.0)
!    ctrl_vect_tmp(181:240) = (0.0,0.0)
!    ctrl_vect_tmp(241:300) = (0.0,0.0)
     
!     DO z = 1, nvars*nlevs
!      PRINT*, z, ev_tmp_arr(z,140), ev_tmp_arr(70,z)
!     END DO
     
!     STOP
     
     ! Eigenvectors are columns of ev_tmp_arr(:, ev)
     
!    ev_tmp_arr(1:nvars*nlevs,   1:60 ) = 0.0
!    ev_tmp_arr(1:nvars*nlevs,  61:120) = 0.0
!    ev_tmp_arr(1:nvars*nlevs, 121:180) = 0.0
!    ev_tmp_arr(1:nvars*nlevs, 181:240) = 0.0
!    ev_tmp_arr(1:nvars*nlevs, 241:300) = 0.0


     ! Eigenvectors are rows of ev_tmp_arr(ev,:) not correct but seems to work???

!    ev_tmp_arr(  1: 60, 1:nvars*nlevs) = 0.0
!   ev_tmp_arr( 61:120, 1:nvars*nlevs) = 0.0
!    ev_tmp_arr(121:180, 1:nvars*nlevs) = 0.0
!    ev_tmp_arr(181:240, 1:nvars*nlevs) = 0.0
!   ev_tmp_arr(241:300, 1:nvars*nlevs) = 0.0
    ...
    END IF
    x_tmp_arr = MATMUL(TRANSPOSE(ev_tmp_arr), ctrl_vect_tmp)
...
!===================================================================================================
 ...
 !   ctrl_vect_tmp =  x_tmp_arr
    ...
    !--------------------------------------------
    DO j = 1, nvars*nlevs
      ctrl_vect_in_hat(j,k) = ctrl_vect_tmp(j)*CONJG(std_eigvecs(j,k))
    END DO 
   ENDDO      
 ...
 IF (herm .EQ. 1) THEN   
   ...
   DO m = 1, nlevs
     ctrl_vect_in_hat (m,1)               = REAL(ctrl_vect_in_hat (m,1))
     ctrl_vect_in_hat (m,181)             = REAL(ctrl_vect_in_hat (m,181))
     ctrl_vect_in_hat (m + nlevs,1)       = negimag * (AIMAG(ctrl_vect_in_hat (m + nlevs,1)))
     ctrl_vect_in_hat (m + nlevs,181)     = negimag * (AIMAG(ctrl_vect_in_hat (m + nlevs,181)))
     ctrl_vect_in_hat (m + 2*nlevs,1)     = negimag * (AIMAG(ctrl_vect_in_hat (m + 2*nlevs,1)))
     ctrl_vect_in_hat (m + 2*nlevs,181)   = negimag * (AIMAG(ctrl_vect_in_hat (m + 2*nlevs,181)))
     ctrl_vect_in_hat (m + 3*nlevs,1)     = REAL(ctrl_vect_in_hat (m + 3*nlevs,1))
     ctrl_vect_in_hat (m + 3*nlevs,181)   = REAL(ctrl_vect_in_hat (m + 3*nlevs,181))
     ctrl_vect_in_hat (m + 4*nlevs,1)     = REAL(ctrl_vect_in_hat (m + 4*nlevs,1))
     ctrl_vect_in_hat (m + 4*nlevs,181)   = REAL(ctrl_vect_in_hat (m + 4*nlevs,181))
   ENDDO

   DO m = 1, nlevs
   kk = 360
     DO k = 2,nlongs/2
       ctrl_vect_in_hat (m,kk)           = -1.0 * (ctrl_vect_in_hat(m,k))
       ctrl_vect_in_hat (m + nlevs,kk)   =        (ctrl_vect_in_hat(m + nlevs,k))
       ctrl_vect_in_hat (m + 2*nlevs,kk) =        (ctrl_vect_in_hat(m + 2*nlevs,k))
       ctrl_vect_in_hat (m + 3*nlevs,kk) = -1.0 * (ctrl_vect_in_hat(m + 3*nlevs,k))
       ctrl_vect_in_hat (m + 4*nlevs,kk) = -1.0 * CONJG(ctrl_vect_in_hat(m + 4*nlevs,k))
       kk = kk-1
     ENDDO
   ENDDO
