...
!===================================================================================================
 ...
 DO k = 1, nlongs
    ...
    IF (ktemp .EQ. 0) THEN
     ...
     ELSE
       ctrl_state_out % u(1:nlevs,k) = negimag * (ctrl_state_out % u(1:nlevs,k) * domain ) / (2.0*REAL(ktemp)*pi)
    ENDIF
 ENDDO
...
! ctrl_state_in % r = ctrl_state_in % r / Cz
 ...
 DO k = 1, nlongs
  ...
  ELSE
     ctrl_state_in % u(1:nlevs,k) = imag * 2.0 * (REAL(ktemp)) * pi * &
...
IMPLICIT NONE
 ...
 DO i = 1, nlongs                    
 ! For each horizontal wave number, k =i
...
SUBROUTINE Eig_inv(ctrl_state_in, ctrl_vect_out, EigVecs, gen_scale, std_eigvecs)
 ...
 DO k = 1,360  ! For each horizontal wavenumber
   ...
   ktemp = k - 1 - (nlongs/2)
   IF (ktemp .EQ. 0) THEN
     ctrl_state_in % u(1:nlevs,k) = ctrl_state_in % u(1:nlevs,k) * imag 
     ctrl_state_in % w(1:nlevs,k) = ctrl_state_in % w(1:nlevs,k) * imag 
     ctrl_state_in % r(1:nlevs,k) = ctrl_state_in % r(1:nlevs,k) * sqrtb
     ctrl_state_in % b(1:nlevs,k) = ctrl_state_in % b(1:nlevs,k) * sqrt(Nsq)
   ELSE
     ctrl_state_in % u(1:nlevs,k) = ctrl_state_in % u(1:nlevs,k) * imag 
     ctrl_state_in % w(1:nlevs,k) = ctrl_state_in % w(1:nlevs,k) * imag *  ( 2 * REAL(ktemp) * pi)/ domain
     ctrl_state_in % r(1:nlevs,k) = ctrl_state_in % r(1:nlevs,k) * (2 *REAL(ktemp) * pi * sqrtb ) / domain
     ctrl_state_in % b(1:nlevs,k) = ctrl_state_in % b(1:nlevs,k) * (2 * REAL(ktemp) * pi * sqrt(Nsq)) / domain 
   ENDIF
...
! INTEGER,                   INTENT(IN)        :: vscale   
 ! Declare Local Variables
 !-----------------------------
 COMPLEX(wp)                                  :: imag, negimag
 REAL(wp)                                     :: rho0, sqrtb, recipdom
 INTEGER                                      :: k, ktemp,i,m,z,mm, kk, herm, remove
 REAL(wp)                                     :: ev_tmp_arr(1:nvars*nlevs, 1:nvars*nlevs) 
 COMPLEX(wp)                                  :: x_tmp_arr(1:nvars*nlevs), ctrl_vect_tmp(1:nvars*nlevs)
...
!    x_tmp_arr = ctrl_vect_tmp
 ...
 DO k = 1, nlongs
     ktemp = k - 1 - (nlongs/2)
    IF (ktemp .EQ. 0) THEN
       ctrl_state_out % u(1:nlevs,k) = ctrl_state_out % u(1:nlevs,k) * negimag
       ctrl_state_out % w(1:nlevs,k) = ctrl_state_out % w(1:nlevs,k) * negimag
    ...
    ELSE
       ctrl_state_out % u(1:nlevs,k) = ctrl_state_out % u(1:nlevs,k) * negimag
       ctrl_state_out % w(1:nlevs,k) = ctrl_state_out % w(1:nlevs,k) * negimag * domain  /   &
                                       (2.0 * pi * REAL(ktemp))  
       ctrl_state_out % r(1:nlevs,k) = ctrl_state_out % r(1:nlevs,k) * domain   /            &
                                       (2.0 * pi * REAL(ktemp) * sqrtb )
       ctrl_state_out % b(1:nlevs,k) = ctrl_state_out % b(1:nlevs,k) * domain   /            &
                                       (2.0 * pi * REAL(ktemp) * sqrt(Nsq) )
    ENDIF
  ENDDO
 END IF
 !==================================================================================================
...
! INTEGER,                   INTENT(IN)        :: vscale
 ...
 COMPLEX(wp)                                  :: imag, negimag
 REAL(wp)                                     :: rho0, sqrtb, recipdom
 INTEGER                                      :: k, ktemp,i,m,z,mm, kk, herm, j, k2, remove
 REAL(wp)                                     :: ev_tmp_arr(1:nvars*nlevs, 1:nvars*nlevs) 
 COMPLEX(wp)                                  :: x_tmp_arr(1:nvars*nlevs), ctrl_vect_tmp(1:nvars*nlevs)
 ...
 DO k = 1,nlongs
   ...
   IF (remove .EQ. 0) THEN
    ...
    ktemp = k - 1 - (nlongs/2)
    IF (ktemp .EQ. 0) THEN
       ctrl_state_out_hat % u(1:nlevs,k) = ctrl_state_out_hat % u(1:nlevs,k) * imag
       ctrl_state_out_hat % w(1:nlevs,k) = ctrl_state_out_hat % w(1:nlevs,k) * imag
    ...
    ELSE
       ctrl_state_out_hat % u(1:nlevs,k) = ctrl_state_out_hat % u(1:nlevs,k) * imag
!
       ctrl_state_out_hat % w(1:nlevs,k) = ctrl_state_out_hat % w(1:nlevs,k) * imag * domain /  &
                                           ( 2.0 * pi * REAL(ktemp) )
!
       ctrl_state_out_hat % r(1:nlevs,k) = ctrl_state_out_hat % r(1:nlevs,k) * domain /         &
                                           ( 2.0 * pi * REAL(ktemp) * sqrtb )
!
       ctrl_state_out_hat % b(1:nlevs,k) = ctrl_state_out_hat % b(1:nlevs,k) * domain /         &
                                           ( 2.0 * pi * REAL(ktemp) * sqrt(Nsq) )
...
!        ctrl_vect_in_hat(j,k) = ctrl_vect_tmp(j)*std_eigvecs(j,k)
  ...
  !=================================================================================================
   ...
   kk = 360
     DO k2 = 2,nlongs/2
       ctrl_vect_in_hat (m,kk)           = -1.0 * (ctrl_vect_in_hat(m,k2))
       ctrl_vect_in_hat (m + nlevs,kk)   =        (ctrl_vect_in_hat(m + nlevs,k2))
       ctrl_vect_in_hat (m + 2*nlevs,kk) =        (ctrl_vect_in_hat(m + 2*nlevs,k2))
       ctrl_vect_in_hat (m + 3*nlevs,kk) = -1.0 * (ctrl_vect_in_hat(m + 3*nlevs,k2))
       ctrl_vect_in_hat (m + 4*nlevs,kk) = -1.0 * CONJG(ctrl_vect_in_hat(m + 4*nlevs,k2))
       kk = kk-1
     ENDDO
   ENDDO

   END IF
   END DO    ! End loop over horizontal wave numbers   
