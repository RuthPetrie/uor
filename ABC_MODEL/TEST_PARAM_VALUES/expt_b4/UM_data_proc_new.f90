...
!---------------------
 CALL check(nf90_open(filename, nf90_nowrite, ncid), 'opening file')

 CALL check(nf90_inq_varid(ncid, 'x', dimidLongs_u), 'inq longs u')
 CALL check(nf90_inq_varid(ncid, 'x_1', dimidLongs_v), 'inq longs v')
 CALL check(nf90_inq_varid(ncid, 'hybrid_ht_3', dimidHalf_levs), 'inq half levs')
 CALL check(nf90_inq_varid(ncid, 'hybrid_ht_2', dimidFull_levs), 'inq full levs')
  
 CALL check(nf90_get_var(ncid, dimidLongs_u, umdata % longs_u(1:nlongs)), 'get longs u')
 CALL check(nf90_get_var(ncid, dimidLongs_v, umdata % longs_v(1:nlongs)), 'get longs v')
 CALL check(nf90_get_var(ncid, dimidHalf_levs, umdata % half_levs(1:nlevs+1)), 'get half levs')
 CALL check(nf90_get_var(ncid, dimidFull_levs, umdata % full_levs(0:nlevs)), 'get full levs')


! Get the variable ids
!----------------------

 CALL check(nf90_inq_varid(ncid, 'u', varidu), 'var inq u')
 CALL check(nf90_inq_varid(ncid, 'v', varidv), 'var inq v')
 CALL check(nf90_inq_varid(ncid, 'dz_dt', varidw), 'var inq w')
 CALL check(nf90_inq_varid(ncid, 'unspecified', variddensity), 'var inq density')
 CALL check(nf90_inq_varid(ncid, 'theta', varidtheta), 'var inq theta')
 CALL check(nf90_inq_varid(ncid, 'ht', varidorog), 'var inq orography')
 CALL check(nf90_inq_varid(ncid, 'field7', varidexpres), 'var inq Exner pressure')

 CALL check(nf90_get_var(ncid, varidu, umdata % u(1:nlongs, 1:nlevs) ), 'get var u')
 CALL check(nf90_get_var(ncid, varidv, umdata % v(1:nlongs, 1:nlevs) ), 'get var v')
 CALL check(nf90_get_var(ncid, varidw, umdata % w(1:nlongs, 0:nlevs-1) ), 'get var w')
 CALL check(nf90_get_var(ncid, variddensity, umdata % density(1:nlongs, 1:nlevs) ), 'get var density')
 CALL check(nf90_get_var(ncid, varidtheta, umdata % theta(1:nlongs, 1:nlevs) ), 'get var theta')
 CALL check(nf90_get_var(ncid, varidorog,  umdata % orog_height(1:nlongs) ), 'get var orography')
 CALL check(nf90_get_var(ncid, varidexpres, umdata % exner_pressure(1:nlongs, 1:nlevs+1) ), 'get var express')
