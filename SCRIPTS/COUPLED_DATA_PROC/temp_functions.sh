...
extract_monthly_data () {
  ...
  for ENS in {1..30}; do
    ...
    for MON in ${MONTHS}; do
      cdo selname,${TEMP} *.pm*${MON}.nc ${MEDIR}temp_${ENS}_${MON}.nc
    done   
  done
...
calc_monthly_zm_stats () {
  ...
  for MON in ${MONTHS}; do
    echo $MON
    for ENS in {1..30}; do
    cdo zonmean ${VARNAME}_${ENS}_${MON}.nc ${VARNAME}_zm_${ENS}_${MON}.nc
    done
    
    cdo ensmean ${VARNAME}_zm_??_${MON}.nc ${MDIR}${VARNAME}_zm_${MON}_ensmean.nc    
    cdo ensstd  ${VARNAME}_zm_??_${MON}.nc ${MDIR}${VARNAME}_zm_${MON}_ensstd.nc    
  done  
}
###########################################################################
