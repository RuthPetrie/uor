...
extract_monthly_data () {
  ...
  for ENS in {1..30}; do
    ...
    for MON in ${MONTHS}; do
      cdo selname,${UMNAME} *.pd*${MON}.nc ${MEDIR}${VARNAME}_${ENS}_daily.nc
      cdo timmean ${MEDIR}${VARNAME}_${ENS}_daily.nc ${MEDIR}${VARNAME}_${ENS}_${MON}.nc
      rm -f ${MEDIR}${VARNAME}*daily.nc
    done   
  done
}
###########################################################################
