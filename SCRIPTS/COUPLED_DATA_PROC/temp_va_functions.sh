...
extract_monthly_data () {
  ...
  for ENS in {1..30}; do
    ...
    for MON in ${MONTHS}; do
      cdo selname,${UMNAME} *.pm*${MON}.nc ${MEDIR}${VARNAME}_${ENS}_${MON}_tmp.nc
    done   
  done
  ...
  for ENS in {1..30}; do
    for MON in ${MONTHS}; do
      cdo sellevel,850,700,600,500 ${MEDIR}${VARNAME}_${ENS}_${MON}_tmp.nc ${MEDIR}${VARNAME}_${ENS}_${MON}_tmp2.nc
      cdo vertmean,0.286 ${MEDIR}${VARNAME}_${ENS}_${MON}_tmp2.nc ${MEDIR}${VARNAME}_${ENS}_${MON}.nc
    done
  done 
  rm -f *tmp*   
